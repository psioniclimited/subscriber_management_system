...
$(document).ready(function () {
    ...
    $('#edit_set_top_box_form').validate({
    ...
    $.get( "{{URL::to('/select/settopboxdetails')}}", { set_top_box_id: $('#set_top_box_id').val() }, function( data ) {
        // Set top box
        stb_model.select2({
            data: [
                {   id: data[0].set_top_box_model.id, 
                    text:  data[0].set_top_box_model.name 
                }
            ],
            placeholder: "Select a set top box model",
            allowClear: true,
            ajax: {
                dataType: 'json',
                url: "{{URL::to('/')}}/select/settopboxmodels",
                delay: 250,
        ...
        <div class="col-md-6">
            <div class="box box-info">
                <div class="box-header with-border">
                    <h3 class="box-title">Edit Set Top Box</h3>
                </div><!-- /.box-header -->
                <!-- form starts here -->
                {!! Form::open(array('url' => array('edit_set_top_box_process', $set_top_box->id), 'id' => 'edit_set_top_box_form', 'method'=>'PUT')) !!}
                <div class="box-body">
                    <div class="form-group">
                        <label>Box Number*</label>
                        <input type="text" class="form-control" id="number" name="number" placeholder="Enter box number" value="{{$set_top_box->number}}">
                    </div>
                    <div class="form-group">
                        ...
                        <div class="input-group">
                            <select class="form-control" name="stb_brands_id" id="stb_brands_id">
                                @foreach($set_top_box_brand as $stbb)
                                    <option value="{{$stbb->id}}" @if($stbb->id == $set_top_box->set_top_box_model->stb_brands_id) selected @endif>{{$stbb->name}}</option>
                                @endforeach
                            </select>
                    ...
                    @endif
                    <input type="hidden" id="set_top_box_id" name="set_top_box_id" value="{{$set_top_box->id}}">
                </div><!-- /.box-body -->
